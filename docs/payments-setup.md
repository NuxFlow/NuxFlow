...
| Auto-sync | Tier creation/update automatically creates products and prices in configured providers |
...
If a payment provider is already configured when you save, NuxFlow automatically creates the matching product and price in that provider. The tier card will show a **Synced** badge when this succeeds.
...
5. Stripe fires `customer.subscription.created` → NuxFlow activates the subscription.
6. Reload the gated page — it should now be accessible.

**Lemon Squeezy test flow:**
1. Use a [Lemon Squeezy test store](https://docs.lemonsqueezy.com/help/getting-started/test-mode).
2. Tiers auto-sync on creation (product + variant created in LS).
3. Complete the LS checkout with a test card.
4. LS fires `subscription_created` → NuxFlow activates the subscription.
