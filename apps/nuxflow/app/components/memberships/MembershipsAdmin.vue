...
const { data: subsData } = await useFetch<{ subscribers: Subscriber[] }>('/api/v1/memberships/subscribers')
...
function syncBadge(tier: Tier) {
  if (tier.price === 0) return { label: 'Free', color: 'neutral' as const }
...
function syncTitle(tier: Tier) {
  const parts: string[] = []
  if (tier.stripePriceId) parts.push('Stripe')
  if (tier.lsVariantId) parts.push('Lemon Squeezy')
  if (tier.paddleProductId) parts.push('Paddle')
  return parts.length ? `Synced to: ${parts.join(', ')}` : 'Not synced to any payment provider'
}
          ...
          <div class="flex items-start justify-between gap-4">
            <div class="min-w-0">
              <div class="flex items-center gap-2 flex-wrap">
                ...
                <UBadge
                  :color="syncBadge(tier).color"
                  variant="soft"
                  size="xs"
                  :title="syncTitle(tier)"
                >
                  {{ syncBadge(tier).label }}
                </UBadge>
              </div>
          ...
          <div class="border-t border-gray-100 dark:border-gray-800 pt-3">
            ...
            <div v-show="showAdvanced" class="mt-2 space-y-3 pl-1">
              <p class="text-xs text-gray-400">
                Leave blank to let NuxFlow auto-sync with configured providers. Fill in manually only if you are
                migrating existing products or need to override the auto-sync.
              </p>
