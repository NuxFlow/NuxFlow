...
// Shared by generate-canvas.post.ts (single-page, synchronous, editor-facing "Generate
// page…" modal) and site-generation.ts (multi-page, job-queue-backed "Generate site…" flow)
// — both need exactly this same block-catalog-constrained generation, just called from
// different places (one request/response cycle vs. one call per planned page in a
// background loop). Extracted here so the prompt/schema can't drift between the two paths.
...
export async function generateCanvasBlocks(
  model: LanguageModel,
  description: string,
  tone: string,
  pageGoal: string,
): Promise<GeneratedCanvasContent> {
  const prompt = `Generate a ${pageGoal} page for: "${description}". Tone: ${tone}.`

  const { object } = await callAiOrThrow(() =>
    generateObject({ model, schema: responseSchema, system: SYSTEM_PROMPT, prompt }),
  )

  return {
    type: 'canvas',
    blocks: object.blocks.map(b => ({ id: ulid(), type: b.type, props: b.props })),
  }
}
