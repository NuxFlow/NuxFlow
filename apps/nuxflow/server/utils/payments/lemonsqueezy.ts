...
export class LemonSqueezyProvider {
  ...
  async createProduct(name: string, description?: string) {
    return this.request<{ data: { id: string; attributes: { name: string } } }>('/products', {
      method: 'POST',
      body: JSON.stringify({
        data: {
          type: 'products',
  ...
  async createVariant(productId: string, name: string, price: number, interval: 'month' | 'year' | 'one_time') {
    const isSubscription = interval !== 'one_time'
    return this.request<{ data: { id: string; attributes: { price: number } } }>('/variants', {
      method: 'POST',
      body: JSON.stringify({
        data: {
          type: 'variants',
          attributes: {
            ...
            ...(isSubscription
              ? {
                  ...
                  subscription_interval_unit: interval === 'month' ? 'month' : 'year',
                }
              : {}),
            status: 'active',
          },
