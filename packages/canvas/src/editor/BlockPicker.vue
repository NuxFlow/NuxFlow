...
// Registry entries for dynamic blocks that don't have a full definition.
const registryDynamic = computed(() => {
  const defIds = new Set(dynamicDefs.value.map(b => b.id))
  return (registry?.dynamicBlocks() ?? []).filter(b => !defIds.has(b.id))
})

// Combined extensions section — definition-registered first, then registry-only.
const extensionBlocks = computed(() => [
  ...dynamicDefs.value,
  ...registryDynamic.value.map(b => ({ ...b, thumbnailColor: '#f0fdf4' as string | undefined })),
])
</script>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-full max-w-xl max-h-[85vh] flex flex-col overflow-hidden">
      ...
      <div class="overflow-y-auto flex-1 min-h-0 px-5 py-4 space-y-6">
        <div v-for="cat in categories" :key="cat">
          <template v-if="blocksFor(cat).length > 0">
        ...
        <!-- Extensions — blocks from installed dynamic plugins only -->
        <div v-if="extensionBlocks.length > 0">
          <p class="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-2">Extensions</p>
          <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
            <button
              v-for="block in extensionBlocks"
              :key="block.id"
              class="flex flex-col items-center gap-2 rounded-lg border border-gray-200 dark:border-gray-700 p-3 hover:border-primary-400 hover:shadow-sm transition-all text-center group"
