...
Auth is handled by Better Auth. There are two Better Auth instances in this codebase and that's deliberate, not an accident — know which one you're touching:

- **`server/utils/better-auth.ts`** (`getOrCreateBetterAuth(event)`) is the real one. `server/api/auth/[...all].ts` and `server/middleware/04.auth-override.ts` route every `/api/auth/**` request to it exclusively, so it's what actually signs users in, sets session cookies, sends password-reset emails, and performs Google/GitHub OAuth. Its instance is cached per incoming `Host` header (not a single shared slot) for 5 minutes, because `socialProviders` credentials can differ per site — see below.
- **`server/auth.config.ts`** (via `@onmax/nuxt-better-auth`'s `defineServerAuth`) exists only so that module can bootstrap its client-side composables (`useUserSession()`, `signIn()`, the passkey client's types). It never serves real `/api/auth/**` traffic. Its `socialProviders` block is type scaffolding only (empty client id/secret, `enabled: false`) — needed because the client's `signIn.social()` provider-name type is inferred from this instance's configured provider keys, not because the values do anything. Don't add real logic to this file — a previous version had its own (subtly different) origin/cookie resolution and its own copy of the password-reset email flow, both silently duplicating `better-auth.ts` in a way that drifted out of sync and broke session validation in production. Don't reintroduce that.

Sessions are read server-side via `requireSession(event)` (throws 401 if absent) or `getAuthSession(event)` (returns `null`), both from `server/utils/auth.ts` — never the `@onmax/nuxt-better-auth`-provided `requireUserSession`/`getUserSession`, which resolve the *other* (fake) instance above.

**`@onmax/nuxt-better-auth` is deliberately pinned to the `alpha` dist-tag (`package.json`), not the `0.1.2` stable release — known issue, do not casually bump it.** The real `better-auth` core package (and `@better-auth/passkey`) are on `1.7.2` and unaffected by any of this; only this thin client-composable-bootstrap module is stuck.

- **What breaks:** on `0.1.2`, `nuxt typecheck` fails with `error TS2322: Type '"PATCH"' is not assignable to type '"GET" | "get" | undefined'` (and the same for `DELETE`) on plain `$fetch(...)` calls elsewhere in the app — e.g. `app/pages/admin/content/[id].vue` and `.../content/index.vue`. Confirmed trigger condition: it only hits API routes that have a `GET` handler **and** other methods (`PATCH`/`DELETE`) registered at the same path (`/api/v1/content/:id` has `get`+`patch`+`delete` → breaks; `/api/v1/users/:id` and `/api/v1/redirects/:id`, which have no `get` at that path, don't). Something in `0.1.2`'s generated ambient types corrupts `$fetch`'s method-literal inference project-wide for those routes — it is not scoped to the module's own auth endpoints.
- **Already ruled out** (each tested in isolation and in combination, against a real `pnpm typecheck` run — not guesses): the module's own `better-auth` core dependency (bumping just that, with `@onmax/nuxt-better-auth` still on `alpha`, typechecks clean); the `useFetch`/`useLazyFetch` overloads the module declaration-merges into `nuxt/app` (removed them directly from the installed package as a test — bug persisted); a genuinely-dropped `#nuxt-better-auth` tsconfig path mapping in `0.1.2`'s `prepare:types` hook that *is* a real, separate regression from the `alpha` version (manually restored it via a `nuxt.config.ts` `alias` — bug persisted anyway).
- **This is a repeat, not a one-off:** `nuxt-modules/better-auth` PR/issue #374 ("fix(types): avoid global auth endpoint inference") fixed an earlier instance of this exact bug class — "unrelated Nitro route inference collapsing in downstream apps" — back in June. Whatever's left in `0.1.2` is a different, still-open manifestation of the same underlying fragility in how the module generates its ambient types, not something a small local patch reliably fixes.
- **Next step, when picking this back up:** file a precise repro against `nuxt-modules/better-auth` (a route with `GET` + another method at the same path, called via `$fetch` with the non-GET method — method type narrows to `"GET" | "get"` only) and re-attempt the bump once there's an upstream fix. Don't spend more time guessing at local workarounds without new information — two independent, targeted attempts already failed.
