...
- **`server/utils/better-auth.ts`** (`getOrCreateBetterAuth(event)`) builds and caches (per incoming `Host` header, 5-minute TTL) the single real `betterAuth({...})` instance. `server/api/auth/[...all].ts` and `server/middleware/04.auth-override.ts` route every `/api/auth/**` request to it — it's what signs users in, sets session cookies, sends password-reset emails, and performs Google/GitHub OAuth. Cached per-`Host` rather than a single shared slot because `socialProviders` credentials can differ per site — see below.
